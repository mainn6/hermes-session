...
S.append(s('02 구성','필요한 부품은 셋','<table class="kv"><tr><td>컴퓨터</td><td>Mac 또는 Windows. 에이전트가 여기서 돌아갑니다</td></tr><tr><td>텔레그램</td><td>봇 하나. 폰에서 명령하고 결과를 받는 창</td></tr><tr><td>모델 키</td><td>에이전트의 두뇌. 저는 Dgrid 한 키로 200개 모델을 골라 씁니다</td></tr></table><p class="note">그 위에 기억(Obsidian 볼트)과 스킬(메일·시트·이미지 같은 작업 묶음)을 얹습니다. 설치는 뒤에 실습으로.</p>'))
S.append(s('03 제가 쓰는 방식','텔레그램 방이 업무 접수창','<p>회사 텔레그램 그룹에 방을 여섯 개 두고, 방마다 스킬을 하나씩 묶었습니다. 사진이나 한 줄을 던지면 그 방의 스킬이 돌아갑니다.</p><table class="kv"><tr><td>업무보고방</td><td>완료·진행·이슈·계획으로 정리 → 구글 Docs 상세 + 시트 요약 행</td></tr><tr><td>회의록방</td><td>메모·녹취·화이트보드 사진 → 회의록 Docs + 시트</td></tr><tr><td>일정방</td><td>“내일 3시 미팅” → 구글 캘린더 이벤트(+Meet)</td></tr><tr><td>명함방</td><td>명함 사진 → 연락처 시트 행</td></tr><tr><td>지출방</td><td>영수증 사진 → 경비 시트 행</td></tr><tr><td>General</td><td>“주간 업무보고” → 위 시트 전체와 캘린더를 종합해 주간보고 Docs</td></tr></table>'))
S.append(s('04 사례 · 업무보고','세 줄 보내면 문서와 시트가 생긴다','<ol class="big"><li>업무보고방에 오늘 한 일 세 줄</li><li>에이전트가 완료 · 진행 · 이슈 · 계획으로 나눠 정리</li><li>“이렇게 저장할게요, 맞나요?” → 확인</li><li>Docs 상세 문서 생성, 시트에 요약과 링크 한 행</li></ol><p class="note">저장 전에 한 번 묻게 둔 이유: 잘못 들어간 행을 나중에 찾는 시간이 더 큽니다. 금요일엔 이 시트들을 모아 주간보고를 뽑습니다.</p>'))
S.append(s('05 사례 · 반복 작업','매일 아침 9시, 시장 요약','<ul class="big"><li>평일 09:00에 코스피·코스닥 전일 지수와 외국인·기관 수급을 읽어 한국어 5줄로 텔레그램에 보냅니다</li><li>만든 방법은 대화 한 줄. “매일 9시에 이거 해서 보내줘. 반복 작업으로 등록해.”</li><li>같은 방식으로 매일 10시 X 초안 세트, 소셜·깃허브 소재 수집이 돌아갑니다</li></ul><p class="note">코드를 짠 게 아니라 일을 말로 정해둔 것입니다. 안 맞으면 말로 고칩니다.</p>'))
S.append(s('06 사례 · 트레이딩','감시는 에이전트, 실행은 사람','<ul class="big"><li>시장·포지션 상태를 읽고 규칙대로 보고하는 감시 에이전트를 따로 둡니다. 긴 대화에 막히지 않게</li><li>규칙 카드: 종목 · 진입 조건 · 손절 · 익절 · 1회 한도 · 금지 조건. 한도를 넘는 주문은 거절하고 사람에게 묻습니다</li><li>대회에서도 같은 방식이었습니다. 15일, $100 시작, 22,936번 거래, 1위. 규칙·기록·손절을 지킨 결과</li></ul>'))
S.append(s('07 사례 · 이미지와 영상','포스터와 영상도 텔레그램에서','<ul class="big"><li>Higgsfield 스킬: “이 레퍼런스 톤으로 포스터 만들어줘” → GPT Image 2.5</li><li>“이 캐릭터가 돌아보는 3D 영상” → Seedance 2.5</li><li>화면 안 글자는 AI에 맡기지 않고 따로 얹습니다. 시안은 480p, 채택분만 1080p</li></ul><p class="note">최근 실물: 투자자 디너 포스터, 소책자 표지 15장, 파트너십 발표 영상 19초. 전부 이 경로로.</p>'))
S.append(s('08 구글 연동','Gmail · 캘린더 · 시트','<ul class="big"><li>메일만 필요하면 Gmail 앱 비밀번호로 2분(himalaya 스킬)</li><li>캘린더·시트·문서까지면 Google Cloud OAuth 한 번(google-workspace 스킬, 5분)</li><li>읽기·요약·초안은 에이전트가, <b>발송과 수정은 확인 후</b></li></ul><pre>“오늘 안 읽은 메일 요약해줘”\n“이 메일에 답장 초안 써줘. 보내기 전에 보여줘”\n“이번 주 일정 겹치는 거 있어?”</pre>'))
S.append(s('09 기억','Obsidian 볼트 = 에이전트의 기억','<ul class="big"><li>에이전트 자체 메모리는 2,200자. 긴 기록은 볼트(markdown 폴더)에 씁니다</li><li>정체성 파일에 한 줄: “프로젝트 질문은 답하기 전에 볼트를 먼저 검색한다”</li><li>밖에서 “그 링크 어디 저장했지?” 하면 파일명을 찍어 답합니다. 저도 같은 파일을 봅니다</li></ul><p class="note">진실은 채팅이 아니라 볼트에 있습니다. 저장 안 된 결정은 없는 결정.</p>'))
S.append(s('10 원칙','회사에서 써도 되는 조건 셋','<ol class="big"><li>위험한 일은 실행 전에 <b>묻는다</b>. 메일 발송 · 결제 · 실거래 · 삭제</li><li>진실은 <b>볼트</b>에. 채팅은 흘러간다</li><li>키는 <b>파일에만</b>. 채팅 · 스크린샷 · 문서에 안 남긴다</li></ol>'))
