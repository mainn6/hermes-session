...
S=[]
# ---- Part 1. 설명 ----
S.append('<section class="slide cover"><div class="kick">AI Agent Operator</div><h1>Hermes Agent<br>알아보고, 실행하고, 활용하기</h1><p class="sub">코드를 짜지 않는 사람이 AI 에이전트를 운영하는 방법. 설명 → 활용 사례 → 실습.</p><p class="tag">AI Agent Operator</p></section>')
S.append(s('01 Hermes Agent','폰에서 시키는 내 AI 비서','<ul class="big"><li>내 컴퓨터에서 돌아가는 오픈소스 에이전트(NousResearch)</li><li>텔레그램에 한 줄 보내면 파일·웹·메일·일정을 읽고 처리해서 답합니다</li><li>자주 하는 일은 한 번 정해두면 정해진 시간에 알아서 돌아갑니다</li><li>컴퓨터 앞에 없어도 됩니다</li></ul>'))
S.append(s('02 비교','OpenClaw와 Hermes','<table class="kv"><tr><td>OpenClaw</td><td>웹사이트를 직접 누르고 입력하는 <b>손</b>. 클릭·입력을 대신한다</td></tr><tr><td>Hermes</td><td>일을 나누고 시키는 <b>비서</b>. “이거 해줘” 하면 처리하고 정리한다</td></tr></table><p class="note">경쟁이 아니라 역할이 다릅니다. 저는 둘을 같이 씁니다.</p>'))
S.append(s('03 활용','실제로 시키는 일','<table class="kv"><tr><td>메일·일정</td><td>받은 메일 요약, 답장 초안, 오늘 일정과 충돌 확인. 발송은 내가 확인하고</td></tr><tr><td>업무 기록</td><td>회의 메모·업무 보고를 정리해 문서와 시트에 저장</td></tr><tr><td>자료 수집</td><td>X·레딧·깃허브에서 AI/Web3 소재를 모아 정리</td></tr><tr><td>트레이딩 감시</td><td>시장·포지션 상태를 읽고 규칙대로 보고. 실거래는 사람이</td></tr><tr><td>이미지·영상</td><td>포스터와 짧은 영상 제작(Higgsfield 스킬: GPT Image 2.5, Seedance 2.5)</td></tr><tr><td>반복 작업</td><td>아침 시장 요약처럼 정해진 시간에 돌아가는 일</td></tr></table>'))
S.append(s('04 구글 연동','Gmail · 캘린더 · 시트','<ul class="big"><li>메일만 필요하면 Gmail 앱 비밀번호로 2분(himalaya 스킬)</li><li>캘린더·시트·문서까지면 Google Cloud OAuth 한 번(google-workspace 스킬, 5분)</li><li>읽기·요약·초안은 에이전트가, <b>발송과 수정은 확인 후</b></li></ul><pre>“오늘 안 읽은 메일 요약해줘”\n“이 메일에 답장 초안 써줘. 보내기 전에 보여줘”\n“이번 주 일정 겹치는 거 있어?”</pre>'))
S.append(s('05 기억','Obsidian 볼트 = 에이전트의 기억','<ul class="big"><li>에이전트 자체 메모리는 2,200자. 긴 기록은 볼트(markdown 폴더)에</li><li>에이전트가 파일로 읽고 쓰고, 나도 같은 파일을 봅니다</li><li>“프로젝트 질문은 답하기 전에 볼트를 먼저 검색” 한 줄이 정체성 파일에 들어갑니다</li></ul><p class="note">연결은 세 단계. 볼트 폴더, 기본 obsidian 스킬 확인, SOUL.md에 경로와 규칙.</p>'))
S.append(s('06 원칙','회사에서 써도 되는 조건','<ol class="big"><li>위험한 일은 실행 전에 <b>묻는다</b>. 메일 발송 · 결제 · 실거래 · 삭제</li><li>진실은 채팅이 아니라 <b>볼트</b>에 있다</li><li>키는 <b>파일에만</b>. 채팅 · 스크린샷 · 문서에 안 남긴다</li></ol>'))
# ---- Part 2. 디스코드 ----
